import heroImg from "@/assets/hero.jpg";
import { useDonation } from "@/contexts/DonationContext";
import { ArrowRight } from "lucide-react";

const HeroSection = () => {
  const { setType, setAmount } = useDonation();

  const scrollToForm = (tab: "monthly" | "once") => {
    setType(tab);
    setAmount(tab === "monthly" ? 30 : 50);
    document.getElementById("formulario-doacao")?.scrollIntoView({ behavior: "smooth" });
  };

  return (
    <section className="relative min-h-screen flex items-center overflow-hidden">
      <div className="absolute inset-0">
        <img src={heroImg} alt="Pessoa sorrindo após tratamento" className="w-full h-full object-cover" width={1920} height={1080} />
        <div className="absolute inset-0 bg-foreground/60" />
      </div>

      <div className="relative container mx-auto px-4 py-32 md:py-0">
        <div className="flex flex-col lg:flex-row items-center gap-10 lg:gap-16">
          {/* Left column */}
          <div className="flex-1 max-w-xl">
            <h1 className="text-4xl md:text-5xl lg:text-6xl font-bold text-primary-foreground leading-tight mb-6">
              Transforme uma{" "}
              <span className="text-secondary">vida.</span>
            </h1>
            <p className="text-lg md:text-xl text-primary-foreground/85 leading-relaxed">
              Quem venceu o câncer não deveria continuar sofrendo sozinho.
              <br className="hidden md:block" />
              Seja parte da cura que vem depois.
            </p>
          </div>

          {/* Right column — Card */}
          <div className="w-full max-w-md bg-background rounded-2xl shadow-2xl overflow-hidden">
            {/* Tab: R$ 1 por dia */}
            <div className="p-6 border-b border-border">
              <h3 className="text-lg font-bold text-foreground mb-2">R$ 1 por dia</h3>
              <p className="text-sm text-muted-foreground leading-relaxed mb-4">
                Com R$ 1 por dia você ajuda a construir o único centro gratuito do Brasil para tratamento de sequelas do câncer.
              </p>
              <button
                onClick={() => scrollToForm("monthly")}
                className="w-full bg-primary text-primary-foreground py-3 rounded-full text-sm font-bold hover:opacity-90 transition-opacity flex items-center justify-center gap-2"
              >
                Quero transformar uma vida agora
                <ArrowRight size={16} />
              </button>
            </div>

            {/* Tab: Doação única */}
            <div className="p-6">
              <h3 className="text-lg font-bold text-foreground mb-2">Doação única</h3>
              <p className="text-sm text-muted-foreground leading-relaxed mb-4">
                Toda contribuição ajuda a construir um espaço onde quem venceu o câncer pode recuperar a vida com dignidade.
              </p>
              <button
                onClick={() => scrollToForm("once")}
                className="w-full border-2 border-primary text-primary py-3 rounded-full text-sm font-bold hover:bg-primary hover:text-primary-foreground transition-colors flex items-center justify-center gap-2"
              >
                Quero fazer minha doação
                <ArrowRight size={16} />
              </button>
            </div>
          </div>
        </div>
      </div>
    </section>
  );
};

export default HeroSection;
